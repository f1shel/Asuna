...
float uniformSpherePdf() { return INV_4PI; }
...
vec3 offsetPositionAlongNormal(vec3 worldPosition, vec3 normal) {
  ...
  return vec3(  //
      abs(worldPosition.x) < origin ? worldPosition.x + floatScale * normal.x
                                    : p_i.x,
      abs(worldPosition.y) < origin ? worldPosition.y + floatScale * normal.y
                                    : p_i.y,
      abs(worldPosition.z) < origin ? worldPosition.z + floatScale * normal.z
                                    : p_i.z);
}

#define CONSISTENTNORMALS
vec3 consistentNormal(const vec3 D, const vec3 iN, const float alpha) {
  // part of the implementation of "Consistent Normal Interpolation", Reshetov
  // et al., 2010 calculates a safe normal given an incoming direction, phong
  // normal and alpha cos(alpha) = dot(shadingNormal, faceNormal)
