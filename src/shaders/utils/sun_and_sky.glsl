...
vec2 mi_lib_square_to_disk(float inout_r, float inout_phi, float in_x,
                           float in_y) {
  // map to [-1, 1] x [-1, 1]
...
vec3 mi_reflection_dir_diffuse_x(vec3 in_normal, vec2 in_sample) {
  ...
  // compute the z component by "lifting" the point onto the unit hemisphere
...
vec3 calc_sun_color(vec3 sun_dir, float turbidity) {
  ...
  vec3 solRad = vec3(1.0 * 127500 / 0.9878, 0.992 * 127500 / 0.9878,
                     0.911 * 127500 / 0.9878);
  if (sun_dir.z > 0.0) {
    float m =
        (1.0f / (sun_dir.z +
                 0.15f * pow(93.885f - acos(sun_dir.z) * 180 / M_PI, -1.253f)));
...
vec3 sky_color_xyz(vec3 in_dir, vec3 in_sun_pos, float in_turbidity,
                   float in_luminance) {
  vec3 xyz;
  float A, B, C, D, E;
  ...
  float zenith_x =
      ((+0.001650f * ts3 - 0.003742f * ts2 + 0.002088f * theta_sun + 0) * t2 +
       (-0.029028f * ts3 + 0.063773f * ts2 - 0.032020f * theta_sun +
        0.003948f) *
           in_turbidity +
       (+0.116936f * ts3 - 0.211960f * ts2 + 0.060523f * theta_sun +
        0.258852f));
  float zenith_y =
      ((+0.002759f * ts3 - 0.006105f * ts2 + 0.003162f * theta_sun + 0) * t2 +
       (-0.042149f * ts3 + 0.089701f * ts2 - 0.041536f * theta_sun +
        0.005158f) *
           in_turbidity +
       (+0.153467f * ts3 - 0.267568f * ts2 + 0.066698f * theta_sun +
        0.266881f));
  xyz.y = in_luminance;
  // TODO: Preetham/Utah
  ...
  float x = (((1.f + A * exp(B / cos_theta)) *
              (1.f + C * exp(D * gamma) + E * cos_gamma * cos_gamma)) /
...
vec3 calc_env_color(vec3 in_sun_dir, vec3 in_dir, float in_turbidity) {
  ...
  float luminance = 1000.0 * ((4.0453 * in_turbidity - 4.9710) * tan(chi) -
  ...
  // use result
  vec3 env_color = vec3(3.241 * XYZ.x - 1.537 * XYZ.y - 0.499 * XYZ.z,
...
vec2 calc_physical_scale(float sun_disk_scale, float sun_glow_intensity,
                         float sun_disk_intensity) {
  float sun_angular_radius = 0.00465f;

  /* This is the angular radius of the sun in radians, scaled according to the
    user's wishes and further scaled by 10 which is the radius of the glow */
  float sun_disk_radius = sun_angular_radius * sun_disk_scale;
  float sun_glow_radius = sun_disk_radius * 10.0f;

  /* The contribution of the sun disk & glow is ultimately driven by these
    expressions: miScalar factor = (1.0f - sun_angle / sun_radius) * 10.0f;
    factor = (miScalar) pow(factor / 10.0, 3.0) * 2.0f * glow_intensity +   //
    SUN GLOW smoothstep(8.5f, 9.5f + (haze / 50.0f), factor) * 100.0f *
    disk_int;     // SUN DISK color.r += data->sun_color.r * factor; color.g +=
    data->sun_color.g * factor; color.b += data->sun_color.b * factor;
  ...
  float actual_sundisk_intensity = sun_disk_intensity * 100.0f *
                                   ...
                                   sundisk_area; /* average value of the */

  /* Apply the proper scaling to get to the target value */
  return vec2((target_sundisk_intensity == 0.0)
                  ? 0.0
                  : target_sundisk_intensity / actual_sundisk_intensity,
              sky_sunglow_scale);
}

float night_brightness_adjustment(vec3 sun_dir) {
  float lmt = 0.30901699437494742410229341718282;
  if (sun_dir.z <= -lmt) return 0.0;
...
vec3 sun_and_sky(in GpuSunAndSky ss, in vec3 in_direction) {
  ...
  vec3 data_sun_color =
      calc_sun_color(sun_dir, downness > 0 ? local_haze : 2.0);
  if (ss.sun_disk_intensity > 0.0 && ss.sun_disk_scale > 0.0) {
    ...
    if (sun_angle < sun_radius) {
      ...
      if (ss.physically_scaled_sun == 1) {
        vec2 return_value = calc_physical_scale(
      ...
      sun_factor = (pow(sun_factor / 10.0, 3.0) * 2.0 * ss.sun_glow_intensity *
                        sky_sunglow_scale +
                    smoothstep(8.5, 9.5 + (local_haze / 50.0), sun_factor) *
                        100.0 * ss.sun_disk_intensity * sky_sundisk_scale);
