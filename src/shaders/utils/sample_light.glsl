...
float pdfEnvmap(in sampler2D envmapSamplers[3], in vec3 direction, in float rotAngle, in vec2 hdrResolution)
{
...
vec3 evalEnvmap(in sampler2D envmapSamplers[3], in vec3 evalDir, in float rotAngle, in float intensity)
{
...
vec3 sampleEnvmap(in uint seed, in sampler2D envmapSamplers[3], in float rotAngle, in vec2 hdrResolution, inout float pdf)
{
  float r1 = rand(seed);
  float r2 = rand(seed);
