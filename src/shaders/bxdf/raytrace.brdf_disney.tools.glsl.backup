...
vec3 EvalDiffuse(float roughness, float metallic, float subsurface, float sheen,
                 vec3 baseColor, vec3 Csheen, vec3 V, vec3 L, vec3 H,
                 out float pdf) {
  pdf = 0.0;
  ...
  pdf = L.z * INV_PI;
  return (1.0 - metallic) *
         (INV_PI * mix(Fd, ss, subsurface) * baseColor + Fsheen);
}

vec3 EvalSpecReflection(float metallic, float roughness, float eta,
                        vec3 specCol, vec3 V, vec3 L, vec3 H, out float pdf) {
  pdf = 0.0;
  if (L.z <= 0.0) return vec3(0.0);
...
vec3 EvalClearcoat(float clearcoatRoughness, float clearcoat, vec3 V, vec3 L,
...
void GetSpecColor(vec3 baseColor, float metallic, float specularTint,
                  float sheenTint, float eta, out vec3 specCol,
                  out vec3 sheenCol) {
  float lum = Luminance(baseColor);
  vec3 ctint = lum > 0.0 ? baseColor / lum : vec3(1.0f);
  float F0 = (1.0 - eta) / (1.0 + eta);
  specCol =
      mix(F0 * F0 * mix(vec3(1.0), ctint, specularTint), baseColor, metallic);
  sheenCol = mix(vec3(1.0), ctint, sheenTint);
}

void GetLobeProbabilities(float metallic, float clearcoat, vec3 baseColor,
                          vec3 specCol, float approxFresnel,
                          out float diffuseWt, out float specReflectWt,
                          out float clearcoatWt) {
...
vec3 DisneySample(in GpuMaterial mat, inout uint seed, vec3 V, vec3 N,
  ...
  // TODO: Tangent and bitangent should be calculated from mesh (provided, the
  // mesh has proper uvs)
  vec3 T, B;
  basis(N, T, B);
  ...
  GetSpecColor(mat.diffuse, mat.metalness, mat.specularTint, mat.sheenTint, 1.5,
               specCol, sheenCol);
  ...
  GetLobeProbabilities(mat.metalness, mat.clearcoat, mat.diffuse, specCol,
                       approxFresnel, diffuseWt, specReflectWt, clearcoatWt);
  ...
  if (r0 < cdf[0])  // Diffuse Reflection Lobe
  {
    ...
    f = EvalDiffuse(mat.roughness, mat.metalness, mat.subsurface, mat.sheen,
                    mat.diffuse, sheenCol, V, L, H, pdf);
    pdf *= diffuseWt;
  ...
  } else  // Specular Reflection/Refraction Lobes
  {
    ...
    f = EvalSpecReflection(mat.metalness, mat.roughness, 1.5, specCol, V, L, H,
                           pdf);
...
vec3 DisneyEval(in GpuMaterial mat, vec3 V, vec3 N, vec3 L, out float bsdfPdf) {
  ...
  GetLobeProbabilities(mat.metalness, mat.clearcoat, mat.diffuse, specCol,
                       fresnel, diffuseWt, specReflectWt, clearcoatWt);
