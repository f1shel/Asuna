...
void PipelineGraphics::createOffscreenResources()
{
  ...
  std::vector<VkImageView> attachments = {m_tColors[0].descriptor.imageView, m_tDepth.descriptor.imageView};
...
void PipelineGraphics::createGraphicsDescriptorSetLayout()
{
  ...
  // Storage images
  outBind.addBinding(OutputBindings::OutputStore, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, NUM_OUTPUT_IMAGES, VK_SHADER_STAGE_ALL);
  // Creation
  ...
  // Camera matrices
  sceneBind.addBinding(SceneBindings::SceneCamera, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
                       VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_RAYGEN_BIT_KHR);
  // Instance description
  sceneBind.addBinding(SceneBindings::SceneInstances, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                       VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR);
  // Textures
  sceneBind.addBinding(SceneBindings::SceneTextures, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                       m_pScene->getTexturesNum(), VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR);
  // Lights
  sceneBind.addBinding(SceneBindings::SceneLights, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
...
void PipelineGraphics::createCameraBuffer()
{
  ...
  m_bCamera = m_alloc.createBuffer(sizeof(GpuCamera), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
...
void PipelineGraphics::updateGraphicsDescriptorSet()
{
  ...
  writesScene.emplace_back(sceneBind.makeWrite(sceneSet, SceneBindings::SceneCamera, &dbiCamera));
  // Instance description
  VkDescriptorBufferInfo dbiSceneDesc{m_pScene->getInstancesDescriptor(), 0, VK_WHOLE_SIZE};
  writesScene.emplace_back(sceneBind.makeWrite(sceneSet, SceneBindings::SceneInstances, &dbiSceneDesc));
  ...
  writesScene.emplace_back(sceneBind.makeWriteArray(sceneSet, SceneBindings::SceneTextures, diit.data()));
  // Lights
  VkDescriptorBufferInfo emittersInfo{m_pScene->getLightsDescriptor(), 0, VK_WHOLE_SIZE};
  writesScene.emplace_back(sceneBind.makeWrite(sceneSet, SceneBindings::SceneLights, &emittersInfo));
  // Writing the information
  vkUpdateDescriptorSets(m_device, static_cast<uint32_t>(writesScene.size()), writesScene.data(), 0, nullptr);
  ...
  for(uint channelId = 0; channelId < NUM_OUTPUT_IMAGES; channelId++)
  {
    VkDescriptorImageInfo imageInfo{{}, m_tColors[channelId].descriptor.imageView, VK_IMAGE_LAYOUT_GENERAL};
    imageInfos[channelId] = imageInfo;
  }
  writesOut.push_back(outBind.makeWriteArray(outSet, OutputBindings::OutputStore, imageInfos.data()));
  vkUpdateDescriptorSets(m_device, static_cast<uint32_t>(writesOut.size()), writesOut.data(), 0, nullptr);
}
