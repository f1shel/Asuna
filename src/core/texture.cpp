...
static void writeImageEXR(const std::string& name,
                          const float*       pixels,
                          int                xRes,
                          int                yRes,
                          int                totalXRes,
                          ...
                          int                yOffset)
{
  ...
  for(int i = 0; i < xRes * yRes; ++i)
    hrgba[i] =
        Rgba(pixels[4 * i], pixels[4 * i + 1], pixels[4 * i + 2], pixels[4 * i + 3]);
...
EnvMapAlloc::EnvMapAlloc(ContextAware* pContext, EnvMap* pEnvmap, const VkCommandBuffer& cmdBuf)
{
      ...
      static_cast<uint64_t>(imgSize.width) * imgSize.height * 4 * sizeof(float);
  {
    auto imageCreateInfo =
        nvvk::makeImage2DCreateInfo(imgSize, format, VK_IMAGE_USAGE_SAMPLED_BIT, true);
    nvvk::Image image =
        m_alloc.createImage(cmdBuf, bufferSize, pEnvmap->getData(), imageCreateInfo);
    nvvk::cmdGenerateMipmaps(cmdBuf, image.image, format, imgSize,
                             imageCreateInfo.mipLevels);
    VkImageViewCreateInfo ivInfo =
        nvvk::makeImageViewCreateInfo(image.image, imageCreateInfo);
  ...
  }
  {
    auto imageCreateInfo =
        nvvk::makeImage2DCreateInfo(imgSize, format, VK_IMAGE_USAGE_SAMPLED_BIT, true);
    nvvk::Image image =
        m_alloc.createImage(cmdBuf, bufferSize, pEnvmap->getConditional(), imageCreateInfo);
    nvvk::cmdGenerateMipmaps(cmdBuf, image.image, format, imgSize,
                             imageCreateInfo.mipLevels);
    VkImageViewCreateInfo ivInfo =
        nvvk::makeImageViewCreateInfo(image.image, imageCreateInfo);
  ...
  }
  {
    auto imageCreateInfo =
        nvvk::makeImage2DCreateInfo(imgSize, format, VK_IMAGE_USAGE_SAMPLED_BIT, true);
    nvvk::Image image =
        m_alloc.createImage(cmdBuf, bufferSize, pEnvmap->getMarginal(), imageCreateInfo);
    nvvk::cmdGenerateMipmaps(cmdBuf, image.image, format, imgSize,
                             imageCreateInfo.mipLevels);
    VkImageViewCreateInfo ivInfo =
        nvvk::makeImageViewCreateInfo(image.image, imageCreateInfo);
...
float* readImage(const std::string& imagePath, int& width, int& height, float gamma)
{
  ...
  {
    LOGE(
        "[x] %-20s: textures only support extensions (.hdr .exr .jpg .png) while %s "
        "is passed in",
  ...
  if(ext == ".hdr")
    pixels = (void*)stbi_loadf(imagePath.c_str(), &width, &height, nullptr, STBI_rgb_alpha);
  else if(ext == ".exr")
...
void writeImage(const std::string& imagePath, int width, int height, float* data)
{
  ...
  {
    auto autoDestroyData =
        reinterpret_cast<float*>(STBI_MALLOC(width * height * 4 * sizeof(float)));
