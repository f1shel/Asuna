...
inline VkExtent2D getGridSize(const VkExtent2D& size) {
  return VkExtent2D{(size.width + (GRID_SIZE - 1)) / GRID_SIZE,
                    (size.height + (GRID_SIZE - 1)) / GRID_SIZE};
}

//-----------------------------------------------------------------------------
// The denoiser will take an image, will convert it to a buffer (compatible
// with Cuda) will denoise the buffer, and put back the buffer to an image.
//
// To make this working, it is important that the VkDeviceMemory associated
// with the buffer has the 'export' functionality.

void DenoiserOptix::init(ContextAware* pContext) {
  m_pContext = pContext;

  m_queueIndex = m_pContext->getQueueFamily();
  m_device = m_pContext->getDevice();
  m_physicalDevice = m_pContext->getPhysicalDevice();
  m_memAlloc.init(m_device, m_physicalDevice);
  m_allocEx.init(m_device, m_physicalDevice, &m_memAlloc);
...
//
bool DenoiserOptix::initOptiX(const OptixDenoiserOptions& options,
                              OptixPixelFormat pixelFormat, bool hdr) {
  CUresult cuRes = cuInit(0);  // Initialize CUDA driver API.
  if (cuRes != CUDA_SUCCESS) {
    LOG_ERROR("{}: initOptiX() cuInit() failed --- {}", "OptiX", cuRes);
    return false;
  }
  ...
  if (cuRes != CUDA_SUCCESS) {
    LOG_ERROR("{}: initOptiX() cuCtxCreate() failed --- {}", "OptiX", cuRes);
    return false;
  }

  // PERF Use CU_STREAM_NON_BLOCKING if there is any work running in parallel on
  // multiple streams.
  cuRes = cuStreamCreate(&m_cuStream, CU_STREAM_DEFAULT);
  if (cuRes != CUDA_SUCCESS) {
    LOG_ERROR("{}: initOptiX() cuStreamCreate() failed --- {}", "OptiX", cuRes);
    return false;
  }

  OPTIX_CHECK(optixInit());
  OPTIX_CHECK(optixDeviceContextCreate(m_cudaContext, nullptr, &m_optixDevice));
  OPTIX_CHECK(optixDeviceContextSetLogCallback(m_optixDevice, context_log_cb,
                                               nullptr, 4));
  ...
  OptixDenoiserModelKind modelKind =
      hdr ? OPTIX_DENOISER_MODEL_KIND_HDR : OPTIX_DENOISER_MODEL_KIND_LDR;
  modelKind = OPTIX_DENOISER_MODEL_KIND_AOV;
  OPTIX_CHECK(
      optixDenoiserCreate(m_optixDevice, modelKind, &m_dOptions, &m_denoiser));
  ...
  try {
    ...
    if (m_dIntensity != 0) {
      OPTIX_CHECK(optixDenoiserComputeIntensity(
          m_denoiser, m_cuStream, &layer.input, m_dIntensity, m_dScratchBuffer,
    ...
    OPTIX_CHECK(optixDenoiserInvoke(
        m_denoiser, m_cuStream, &denoiserParams, m_dStateBuffer,
        m_dSizes.stateSizeInBytes, &guide_layer, &layer, 1, 0, 0,
        m_dScratchBuffer, m_dSizes.withoutOverlapScratchSizeInBytes));

    CUDA_CHECK(
        cudaStreamSynchronize(m_cuStream));  // Making sure the denoiser is done

    cudaExternalSemaphoreSignalParams sigParams{};
    sigParams.flags = 0;
    sigParams.params.fence.value = ++fenceValue;
    cudaSignalExternalSemaphoresAsync(&m_semaphore.cu, &sigParams, 1,
                                      m_cuStream);
...
//
void DenoiserOptix::imageToBuffer(const VkCommandBuffer& cmdBuf,
                                  const std::vector<nvvk::Texture>& imgIn) {
#if USE_COMPUTE
  ...
  for (int i = 0; i < static_cast<int>(imgIn.size()); i++) {
    ...
    VkImageSubresourceRange subresourceRange{VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0,
                                             1};
    nvvk::cmdBarrierImageLayout(cmdBuf, imgIn[i].image, VK_IMAGE_LAYOUT_GENERAL,
    ...
    vkCmdCopyImageToBuffer(cmdBuf, imgIn[i].image,
                           VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, pixelBufferIn,
                           1, &region);

    // Put back the image as it was
    nvvk::cmdBarrierImageLayout(cmdBuf, imgIn[i].image,
                                VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
...
//
void DenoiserOptix::bufferToBuffer(const VkCommandBuffer& cmdBuf,
                                   const std::vector<nvvk::Buffer>& bufIn) {
  LABEL_SCOPE_VK(cmdBuf);

  VkDeviceSize buf_size = static_cast<VkDeviceSize>(
      m_sizeofPixel * m_imageSize.width * m_imageSize.height);
  VkBufferCopy region{0, 0, buf_size};

  for (int i = 0; i < static_cast<int>(bufIn.size()); i++) {
    vkCmdCopyBuffer(cmdBuf, bufIn[i].buffer, m_pixelBufferIn[i].bufVk.buffer, 1,
                    &region);
  }
}
...
//
void DenoiserOptix::bufferToImage(const VkCommandBuffer& cmdBuf,
                                  nvvk::Texture* imgOut) {
#if USE_COMPUTE
  ...
  VkImageSubresourceRange subresourceRange{VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0,
                                           1};
  nvvk::cmdBarrierImageLayout(cmdBuf, imgOut->image, VK_IMAGE_LAYOUT_GENERAL,
  ...
  region.imageExtent.height = m_imageSize.height;
  region.imageExtent.depth = 1;
  vkCmdCopyBufferToImage(cmdBuf, pixelBufferOut, imgOut->image,
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);

  // Put back the image as it was
  nvvk::cmdBarrierImageLayout(cmdBuf, imgOut->image,
                              VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                              VK_IMAGE_LAYOUT_GENERAL, subresourceRange);
...
void DenoiserOptix::destroyBuffer() {
  for (auto& p : m_pixelBufferIn) p.destroy(m_allocEx);
...
bool DenoiserOptix::uiSetup() {
  ...
  if (ImGui::CollapsingHeader("Denoiser", ImGuiTreeNodeFlags_DefaultOpen)) {
    modified |=
        ImGuiH::Control::Checkbox("Denoise", "", (bool*)&m_denoisedMode);
    modified |= ImGuiH::Control::Slider(
        "Start Frame", "Frame at which the denoiser starts to be applied",
...
void DenoiserOptix::allocateBuffers(const VkExtent2D& imgSize) {
  ...
  VkDeviceSize bufferSize = static_cast<unsigned long long>(m_imageSize.width) *
                            m_imageSize.height * 4 * sizeof(float);
  VkBufferUsageFlags usage{VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                           VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                           VK_BUFFER_USAGE_TRANSFER_SRC_BIT};

  {  // Color
    m_pixelBufferIn[0].bufVk = m_allocEx.createBuffer(
        bufferSize, usage, VK_MEMORY_HEAP_DEVICE_LOCAL_BIT);
    createBufferCuda(m_pixelBufferIn[0]);  // Exporting the buffer to Cuda
                                           // handle and pointers
  ...
  // Albedo
  {
    m_pixelBufferIn[1].bufVk = m_allocEx.createBuffer(
        bufferSize, usage, VK_MEMORY_HEAP_DEVICE_LOCAL_BIT);
  ...
  // Normal
  {
    m_pixelBufferIn[2].bufVk = m_allocEx.createBuffer(
        bufferSize, usage, VK_MEMORY_HEAP_DEVICE_LOCAL_BIT);
  ...
  // Output image/buffer
  m_pixelBufferOut.bufVk = m_allocEx.createBuffer(
      bufferSize, usage | VK_FORMAT_FEATURE_TRANSFER_SRC_BIT,
  ...
  OPTIX_CHECK(optixDenoiserComputeMemoryResources(
      m_denoiser, m_imageSize.width, m_imageSize.height, &m_dSizes));

  CUDA_CHECK(cudaMalloc((void**)&m_dStateBuffer, m_dSizes.stateSizeInBytes));
  CUDA_CHECK(cudaMalloc((void**)&m_dScratchBuffer,
                        m_dSizes.withoutOverlapScratchSizeInBytes));
  CUDA_CHECK(cudaMalloc((void**)&m_dMinRGB, 4 * sizeof(float)));
  if (m_pixelFormat == OPTIX_PIXEL_FORMAT_FLOAT3 ||
      m_pixelFormat == OPTIX_PIXEL_FORMAT_FLOAT4)
    CUDA_CHECK(cudaMalloc((void**)&m_dIntensity, sizeof(float)));

  OPTIX_CHECK(optixDenoiserSetup(m_denoiser, m_cuStream, m_imageSize.width,
                                 m_imageSize.height, m_dStateBuffer,
                                 m_dSizes.stateSizeInBytes, m_dScratchBuffer,
                                 m_dSizes.withoutOverlapScratchSizeInBytes));
}

//--------------------------------------------------------------------------------------------------
// Get the Vulkan buffer and create the Cuda equivalent using the memory
// allocated in Vulkan
//
void DenoiserOptix::createBufferCuda(BufferCuda& buf) {
  nvvk::MemAllocator::MemInfo memInfo =
      m_allocEx.getMemoryAllocator()->getMemoryInfo(buf.bufVk.memHandle);
#ifdef WIN32
  ...
  CUDA_CHECK(cudaExternalMemoryGetMappedBuffer(
      &buf.cudaPtr, cudaExtMemVertexBuffer, &cudaExtBufferDesc));
}
...
void DenoiserOptix::createSemaphore() {
  ...
  std::memset(&externalSemaphoreHandleDesc, 0,
  ...
  CUDA_CHECK(cudaImportExternalSemaphore(&m_semaphore.cu,
...
void DenoiserOptix::createCopyPipeline() {
  {
    ...
    bind.addBinding(0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1,
                    VK_SHADER_STAGE_COMPUTE_BIT);
    bind.addBinding(1, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1,
                    VK_SHADER_STAGE_COMPUTE_BIT);
    bind.addBinding(2, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1,
                    VK_SHADER_STAGE_COMPUTE_BIT);
    bind.addBinding(3, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                    VK_SHADER_STAGE_COMPUTE_BIT);
    bind.addBinding(4, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                    VK_SHADER_STAGE_COMPUTE_BIT);
    bind.addBinding(5, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                    VK_SHADER_STAGE_COMPUTE_BIT);

    CREATE_NAMED_VK(m_desc[SHD].pool, bind.createPool(m_device, 1));
    CREATE_NAMED_VK(m_desc[SHD].layout, bind.createLayout(m_device));
    CREATE_NAMED_VK(m_desc[SHD].set,
                    nvvk::allocateDescriptorSet(m_device, m_desc[SHD].pool,
                                                m_desc[SHD].layout));
    ...
    vkCreatePipelineLayout(m_device, &pipeInfo, nullptr,
                           &m_pipelines[SHD].layout);
    NAME_VK(m_pipelines[SHD].layout);

    VkPipelineShaderStageCreateInfo stageInfo{
        VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO};
    stageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
    stageInfo.module = nvvk::createShaderModule(
        m_device, nvh::loadFile("../shaders/post.img2buffer.comp.spv", true,
                                {m_pContext->getRoot()}));
    ...
    vkCreateComputePipelines(m_device, {}, 1, &compInfo, nullptr,
                             &m_pipelines[SHD].p);
...

  {
    ...
    bind.addBinding(0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1,
                    VK_SHADER_STAGE_COMPUTE_BIT);
    bind.addBinding(1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                    VK_SHADER_STAGE_COMPUTE_BIT);
    ...
    stageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
    stageInfo.module = nvvk::createShaderModule(
        m_device, nvh::loadFile("../shaders/post.buffer2img.comp.spv", true,
                                {m_pContext->getRoot()}));
    stageInfo.pName = "main";
...
VkWriteDescriptorSet makeWrite(const VkDescriptorSet& set, uint32_t bind,
...
//
void DenoiserOptix::copyImageToBuffer(const VkCommandBuffer& cmd,
                                      const std::vector<nvvk::Texture>& imgIn) {
  LABEL_SCOPE_VK(cmd);
  constexpr uint32_t SHD = 0;
  ...
  VkDescriptorBufferInfo buf0{m_pixelBufferIn[0].bufVk.buffer, 0,
                              VK_WHOLE_SIZE};
  VkDescriptorBufferInfo buf1{m_pixelBufferIn[1].bufVk.buffer, 0,
                              VK_WHOLE_SIZE};
  VkDescriptorBufferInfo buf2{m_pixelBufferIn[2].bufVk.buffer, 0,
                              VK_WHOLE_SIZE};
  ...
  vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                          m_pipelines[SHD].layout, 0, 1, &m_desc[SHD].set, 0,
                          nullptr);
...
//
void DenoiserOptix::copyBufferToImage(const VkCommandBuffer& cmd,
                                      const nvvk::Texture* imgIn) {
  LABEL_SCOPE_VK(cmd);
  constexpr uint32_t SHD = 1;
