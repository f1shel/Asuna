...
#define OPTIX_CHECK(call)                                            \
  do {                                                               \
    ...
    if (res != OPTIX_SUCCESS) {                                      \
      std::stringstream ss;                                          \
      ss << "call (" << #call << " ) failed with code " << res \
         << " (" __FILE__ << ":" << __LINE__ << ")\n";               \
      LOG_ERROR("{}: {}", "OptiX", ss.str());                        \
      exit(-1);                                                      \
    }                                                                \
  } while (false)
...
#define CUDA_CHECK(call)                                              \
  do {                                                                \
    ...
    if (error != cudaSuccess) {                                       \
      std::stringstream ss;                                           \
      ss << "call (" << #call << " ) failed with code " << error \
         << " (" __FILE__ << ":" << __LINE__ << ")\n";                \
      LOG_ERROR("{}: {}", "Cuda", ss.str());                         \
      exit(-1);                                                      \
    }                                                                 \
  } while (false)
...
static void context_log_cb(unsigned int level, const char* tag,
  ...
  if (level == 4)
    LOG_INFO("{}: tag [{}], message --- {}", "OptiX", tag, message);
  else if (level == 3)
    LOG_WARN("{}:tag [{}], message --- {}", "OptiX", tag, message);
  else if (level)
    LOG_ERROR("{}:tag [{}], message --- {}", "OptiX", tag, message);
}
...
struct DenoiserOptix {
  ...
  bool initOptiX(const OptixDenoiserOptions& options,
  ...
  void imageToBuffer(const VkCommandBuffer& cmdBuf,
                     const std::vector<nvvk::Texture>& imgIn);
  void bufferToBuffer(const VkCommandBuffer& cmdBuf,
                      const std::vector<nvvk::Buffer>& bufIn);

  void createCopyPipeline();
  void copyImageToBuffer(const VkCommandBuffer& cmd,
                         const std::vector<nvvk::Texture>& imgIn);
  void copyBufferToImage(const VkCommandBuffer& cmd,
...
private:
  ...
  nvvk::DedicatedMemoryAllocator
      m_memAlloc;  // Using dedicated allocations for simplicity
  nvvk::ExportResourceAllocator
      m_allocEx;  // ResourceAllocator with export flag (interop)
