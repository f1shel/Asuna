...
void PipelinePost::createPostPipeline()
{
	...
	pipelineGenerator.addShader(nvh::loadFile("shaders/post.idle.vert.spv", true, m_pContext->m_root), VK_SHADER_STAGE_VERTEX_BIT);
	pipelineGenerator.addShader(nvh::loadFile("shaders/post.idle.frag.spv", true, m_pContext->m_root), VK_SHADER_STAGE_FRAGMENT_BIT);
