...
void PipelineRaytrace::run(const VkCommandBuffer &cmdBuf)
{
	...
	vkCmdBindDescriptorSets(cmdBuf, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, m_pipelineLayout, 0, (uint32_t) descSets.size(),
...
void PipelineRaytrace::createRtPipeline()
{
	...
	// Raygen
	stage.module =
	    nvvk::createShaderModule(m_device, nvh::loadFile("shaders/raytrace.intersectiontest.rgen.spv", true, m_pContext->m_root));
	stage.stage     = VK_SHADER_STAGE_RAYGEN_BIT_KHR;
	stages[eRaygen] = stage;
	NAME2_VK(stage.module, "Raygen");
	// Miss
	stage.module     = nvvk::createShaderModule(m_device, nvh::loadFile("shaders/raytrace.intersectiontest.rmiss.spv", true,
	                                                                    m_pContext->m_root));
	stage.stage      = VK_SHADER_STAGE_MISS_BIT_KHR;
	stages[eRayMiss] = stage;
	NAME2_VK(stage.module, "RayMiss");
	// Closet hit
	stage.module        = nvvk::createShaderModule(m_device, nvh::loadFile("shaders/raytrace.intersectiontest.rchit.spv", true,
	                                                                       m_pContext->m_root));
