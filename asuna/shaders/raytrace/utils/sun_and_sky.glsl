...
vec3 calc_sun_color(vec3 sun_dir, float turbidity)
{
    ...
    {
        float m =
            (1.0f / (sun_dir.z + 0.15f * pow(93.885f - acos(sun_dir.z) * 180 / M_PI, -1.253f)));
...
vec3 sky_color_xyz(vec3 in_dir, vec3 in_sun_pos, float in_turbidity, float in_luminance)
{
    ...
    float zenith_x = ((+0.001650f * ts3 - 0.003742f * ts2 + 0.002088f * theta_sun + 0) * t2 +
                      (-0.029028f * ts3 + 0.063773f * ts2 - 0.032020f * theta_sun + 0.003948f) *
                          in_turbidity +
    ...
    float zenith_y = ((+0.002759f * ts3 - 0.006105f * ts2 + 0.003162f * theta_sun + 0) * t2 +
                      (-0.042149f * ts3 + 0.089701f * ts2 - 0.041536f * theta_sun + 0.005158f) *
                          in_turbidity +
    ...
    float x = (((1.f + A * exp(B / cos_theta)) *
               ...
               ((1 + A * exp(B / 1.0)) *
                (1 + C * exp(D * theta_sun) + E * cos_theta_sun * cos_theta_sun)));
    ...
    float y =
        (((1 + A * exp(B / cos_theta)) * (1 + C * exp(D * gamma) + E * cos_gamma * cos_gamma)) /
         ((1 + A * exp(B / 1.0)) *
          (1 + C * exp(D * theta_sun) + E * cos_theta_sun * cos_theta_sun)));
...
vec3 calc_env_color(vec3 in_sun_dir, vec3 in_dir, float in_turbidity)
{
    ...
    float luminance =
        1000.0 * ((4.0453 * in_turbidity - 4.9710) * tan(chi) - 0.2155 * in_turbidity + 2.4192);
                         ...
                         float sun_disk_intensity)
{
    ...
    /* approximation! needs to be re-calculated from the integral of
      the function */
    float actual_sundisk_intensity = sun_disk_intensity * 100.0f * actual_sundisk_integral /
...
vec3 sun_and_sky(in SunAndSky ss, in vec3 in_direction)
{
    ...
    if (ss.sun_disk_intensity > 0.0 && ss.sun_disk_scale > 0.0)
    {
        ...
        if (sun_angle < sun_radius)
        {
            ...
            {
                vec2 return_value = calc_physical_scale(ss.sun_disk_scale, ss.sun_glow_intensity,
            ...
            sun_factor =
                (pow(sun_factor / 10.0, 3.0) * 2.0 * ss.sun_glow_intensity * sky_sunglow_scale +
                 smoothstep(8.5, 9.5 + (local_haze / 50.0), sun_factor) * 100.0 *
                     ss.sun_disk_intensity * sky_sundisk_scale);
