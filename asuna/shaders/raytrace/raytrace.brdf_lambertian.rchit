...
void main()
{
  ...
  if(inst.lightId >= 0)
  {
    ...
    vec3     lightDirection = normalize(vec3(hitPos - payload.ray.origin));
    vec3     lightNormal    = normalize(cross(light.u, light.v));
    if(dot(lightNormal, lightDirection) < 0)
      payload.radiance += payload.throughput * light.emittance;
    payload.stop = 1;
    return;
  }
  ...
  /*
  {
    ...
    contrib.visible  = false;
    {
      ...
      if(dot(lightSample.direction, ffnormal) > 0.0 && dot(lightSample.normal, lightSample.direction) < 0)
      {
        ...
        float misWeight = lightSample.shouldMis > 0.0 ? max(0.0, powerHeuristic(lightSample.pdf, bsdfSample.pdf)) : 1.0;

        Li +=
            misWeight * bsdfSampleVal * abs(dot(lightSample.direction, ffnormal)) * lightSample.emittance / lightSample.pdf;
